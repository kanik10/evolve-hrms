...
export function DeleteDialog({ entityType, entityName, open, onOpenChange, onConfirm, isArchived = false }: DeleteDialogProps) {
  return (
      ...
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription asChild>
            <div className="space-y-3 text-sm text-muted-foreground">
              <p>
                This will permanently remove the <span className="font-medium text-foreground">{entityName}</span> {entityType.toLowerCase()} and all associated configurations.
              </p>
              {!isArchived && (
                <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2.5 text-amber-800 dark:border-amber-800/30 dark:bg-amber-900/20 dark:text-amber-400">
                  ...
                  <p className="mt-0.5 text-xs">Archiving preserves historical records while deactivating the {entityType.toLowerCase()}.</p>
                </div>
              )}
            </div>
          </AlertDialogDescription>
        ...
        <AlertDialogFooter className="flex-row gap-2 sm:justify-between">
          ...
          <div className="flex flex-1 gap-2 sm:flex-none">
            {!isArchived && (
              <Button variant="outline" className="flex-1" onClick={() => onConfirm("archive")}>
                <Archive className="mr-2 h-4 w-4" />
                Archive
              </Button>
            )}
            <Button variant="destructive" className="flex-1" onClick={() => onConfirm("delete")}>
